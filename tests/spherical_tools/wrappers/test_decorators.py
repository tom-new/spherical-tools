...
def test_validate_coordinates_accepts_list_and_casts_float64():
...
def test_validate_coordinates_rejects_wrong_last_dim():
    with pytest.raises(ValueError, match="last dimension"):
        validate_coordinates(np.ones(4), ndim=(2, 3), name_in="foo", name_out="bar")


def test_valid_int_ndim_keeps_dtype_and_shape():
...
def test_valid_sequence_ndim_allows_any_of_them():
...
def test_invalid_shape_raises_value_error(bad_arr, ndim):
...
def test_inputs_with_more_dims_pass_if_last_dim_matches():
...
## ensure_units decorator tests
def test_does_not_mutate_input_when_converting_degrees():
...
def test_no_conversion_degrees_false():
    @ensure_units(
        ndim=3,
        name_in="in",
        name_out="out",
...
def test_no_conversion_flags_false_but_degrees_true():
    @ensure_units(
...
def test_both_conversions_with_additional_args():
    @ensure_units(
        ndim=3,
        name_in="in",
        name_out="out",
...
def test_2d_array_convert_input():
    @ensure_units(
